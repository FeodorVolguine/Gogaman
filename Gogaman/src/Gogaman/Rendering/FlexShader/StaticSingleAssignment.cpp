...
namespace Gogaman
{
	namespace FlexShader
	{
		namespace IR
		{
			...
			{
				//BuildControlFlowGraph(intermediateRepresentation);
				...
				for(auto i = 0; i < dominators.size(); i++)
					GM_LOG_CORE_INFO("Immediate dominator of node %d: %d", postOrder[i] + 1, postOrder[dominators[i]] + 1);
				...
				{
					GM_LOG_CORE_INFO("Dominance frontiers of node %d:", postOrder[i] + 1);
					for(auto j : dominanceFrontiers[i])
						GM_LOG_CORE_INFO("	%d", j + 1);
				}
			}

			void SSA::BuildControlFlowGraph(const IR &ir)
			{
				std::unordered_map<uint32_t, uint32_t> instructionExecutionIndices;
				instructionExecutionIndices.reserve(ir.executionOrder.size());
				for(uint32_t i = 0; i < ir.executionOrder.size(); i++)
					instructionExecutionIndices[ir.executionOrder[i]] = i;

				const auto IsBranch = [](const Instruction &instruction) { return (instruction.operation == Operation::Branch) || (instruction.operation == Operation::BranchOnNotEqual); };

				//Generate leaders
				std::set<uint32_t> leaders;
				leaders.emplace(0);
				bool previousInstructionWasBranch = IsBranch(ir.instructions[ir.executionOrder[0]]);
				for(auto i = 1; i < ir.executionOrder.size(); i++)
				{
					if(previousInstructionWasBranch)
						leaders.emplace(i);

					const Instruction &instruction = ir.instructions[ir.executionOrder[i]];
				...
				while(!leaders.empty())
				{
					...
					for(uint32_t i = blockStart + 1; (i < ir.executionOrder.size()) && !leaders.contains(i); i++)
						blockEnd = i;
				...
				{
					const Instruction &endInstruction = ir.instructions[ir.executionOrder[m_BasicBlocks[i].second]];
